...
| 2026-08-18 | **The console shares one auth realm with the app, and MFA becomes a database permission.** AC-1's separate auth realm needs a second Supabase project, which the no-company / no-paid-infrastructure decision above forecloses. In its place: `admin_role()` resolves to nothing unless the session carries `aal2`, so an operator without a second factor is refused by Postgres rather than by a screen; every console function re-checks `admin_roles`; the app is deployed to its own origin. The residual risk is stated in `0009_v3_console_rpcs.sql` — whoever takes over the operator's ordinary account has their console session. Revisit when there is an entity to hold a second project. | [12_CONSOLE.md](12_CONSOLE.md) AC-1's "separate auth realm" |
| 2026-08-20 | **No travel-radius question. Match anywhere in town, ranked by closest distance.** The anchor pin is the distance signal; `max_travel_m` becomes a per-city config bound rather than a person's answer. | [05_PLACES.md](05_PLACES.md) §2's "plus one control: how far are you willing to go" |
| 2026-08-20 | **The availability picker is a calendar, not a list of chips.** A month grid where the operating days are the live cells, each carrying its slot count and the density wash the transcript asked for ("an indicator background behind slot that indicates its a good time to pick"). | The day-block/time-chip list built in P1 |
| 2026-08-20 | **Onboarding does not ask for a city.** Osijek is locked and `people.city_id` is not client-settable by design; the server derives the city from the anchor. | The city-picker onboarding step built in P1 |
| 2026-08-20 | **AAI@EduHr is reachable sooner than [09_OPEN_QUESTIONS.md](09_OPEN_QUESTIONS.md) C-1 claimed.** OIDC is supported (no SAML broker), registration is self-service through `registar.aaiedu.hr`, and `AAI@EduHr Lab` is a full test federation usable before any approval. The organisational gate is real — the SP must belong to a partner or a *matična ustanova* — but the procedure is published, not obtained by email. `hrEduPersonPersistentID` is mandatory and permanent; `hrEduPersonDateOfBirth` is **optional**, so the 18+ gate stays attested. | C-1's "the registration procedure is not published in a form I could verify" and its instruction to email SRCE first |
